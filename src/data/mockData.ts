...
export const filterData = (country: string, brand: string, dateRange: any) => {
  // Since we only have France and Dior, just return the data for this combination
...
const getDataForCountryAndBrand = (country: string, brand: string) => {
  ...
  switch (brand) {
    case 'Dior': brandMultiplier = 1.50; break;
  ...
  const channelFactors = {
    'TV': country === 'France' ? 1.2 : 0.9,
    'Radio': country === 'France' ? 1.15 : 0.95,
    'Print': brand === 'Dior' ? 1.25 : 0.8,
    'Digital': brand === 'Dior' ? 1.3 : 1.0,
    'CRM': country === 'France' ? 1.4 : 1.0,
    'Promo': brand === 'Dior' ? 1.3 : 0.9
  };
